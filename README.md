...
>The above approach has multiple advantages:
    ...
    1. Self Assignment check can be skipped so useful for throwing noexcept

    2. Other is passed to assignment operator as a value and so memory deallocation of \*this previously allocated memory happens automatically

    3. This can take both lvalue and rvalue so implements both the copy assignment and move assignment.
